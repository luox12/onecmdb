...
public class GraphControl2 {
	public void addRelationPath(Node selectedInTree) {
		Node leaf = selectedInTree;
		while(leaf != null) {
			...
			addItem(leaf.getString("alias"), leaf.getString("alias"), primary);
			
			Edge e = leaf.getParentEdge();
			
			if (e != null) {
				e.getSourceNode().getString("alias");
				e.getTargetNode().getString("alias");
				e.getString("alias");
				//addRelation(sourceId, refType, targetId);
			}
		}
		leaf = leaf.getParent();
	}
	...
	private ItemSecurityConstraint getSecurityConstraint() {
	...
	public void update() {
		...
		ItemSecurityConstraint security = getSecurityConstraint();
